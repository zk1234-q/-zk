...
import { getAllMonthlyCategoryBudgets, getShoppingBudgetItems, replaceAllMonthlyCategoryBudgets, replaceAllShoppingBudgetItems } from './budgetStorage';
...
export function validateBackupFile(value: unknown): BackupFile {
  ...
  if (value.appName !== 'expense-bill-analyzer' || (value.schemaVersion !== 2 && value.schemaVersion !== 3 && value.schemaVersion !== 4)) {
...
export async function importLocalData(backup: BackupFile): Promise<void> {
  ...
  await replaceAllShoppingBudgetItems(backup.shoppingBudgetItems);
